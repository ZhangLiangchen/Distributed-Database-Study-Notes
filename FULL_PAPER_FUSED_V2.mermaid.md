...
flowchart LR
 ...
 A2 -->|"即一个 Raft Group"| A3["共识分片"]
 A2 -->|"PD 调度/迁移单元"| A4["调度分片"]
 A2 -->|"单分片事务的天然边界"| A5["事务分片"]
 end
 subgraph G2["OceanBase 4.x：解耦的归属映射"]
 direction TB
 B1["Partition（逻辑分片）"] --> B2["Tablet（物理分片）"]
 B2 -->|"多 Tablet 归属一个 LS"| B3["Log Stream（共识+调度分片）"]
 B3 -->|"LS 是事务参与者"| B4["事务分片"]
 end
 G1 ~~~ G2
```
...
```mermaid
flowchart TB
 subgraph TiDB_F["TiDB 异常路径"]
 direction TB
 A1["热点或大 Region"] --> A2{"split checker 发现阈值/负载?"}
 A2 -->|是| A3["向 PD 请求 split / 生成 operator"]
 A3 --> A4["新 Region 形成新的 Raft Group"]
 A4 --> A5["PD scatter / balance / hot-region 调度"]
 A2 -->|否或单 key 热点| A6["Region split 收益有限(设计边界)"]
 A7["刚 split 的 Region 进 splitCache(TTL=split-merge-interval)"] --> A8["MergeChecker 跳过，避免'拆完即合'"]
 B1["Region Leader 所在 TiKV 宕机"] --> B2["Raft 在剩余副本重新选主(秒级)"]
 B2 --> B3["PD 发现副本不足 → 补副本(snapshot 传输)"]
 B3 --> B4["客户端 Region location 缓存刷新/重试"]
 C1["PD Leader 切换 / PD 失去 quorum"] --> C2["新事务的 TSO 分配 / 调度 / split-merge 暂停"]
 C2 --> C3["已缓存路由的只读/单 Region 写可短时继续(见 §1.7)"]
 end
 subgraph OB_F["OceanBase 4.x 异常路径"]
 direction TB
 D1["LS Leader 所在 OBServer 宕机"] --> D2["Leader lease 失效"]
 D2 --> D3["Follower 在剩余副本选出新 LS Leader"]
 D3 --> D4["该 LS 下所有 Tablet 一起恢复强一致读写(粒度=LS)"]
 E1["少数 Follower 故障且多数派完整"] --> E2["服务保持，副本后续补齐"]
 F1["Tablet transfer / LS 调整进行中"] --> F2["tablet-to-LS 映射变化"]
 F2 --> F3["位置缓存刷新 / SQL 重试；transfer 可取消/重试，归属以 ls_id 为准"]
 G1["RS 异常(__all_core_table leader 所在 OBServer)"] --> G2["RS 随 leader 切换在新 OBServer 重启"]
 G2 --> G3["影响建连 / major compaction；集群内 Paxos 切换官方 RPO=0、RTO<8s(见 §1.7)"]
 end
...
```mermaid
flowchart TB
 subgraph G1["OceanBase 路由的三层结构（§2.3.1）"]
 direction TB
 T1["Table（表）"] --> P1["Partition（逻辑分区）"]
 P1 --> TB1["Tablet（物理存储对象）"]
 TB1 -->|"多 Tablet 归属 1 个 LS"| LS1["Log Stream（LS，共识 / 调度单元）"]
 LS1 --> GL["get_leader：取 LS Leader 地址"]
 LS1 --> GA["get：取 LS 全部副本地址"]
 GL --> RP["副本复制协议：—（本节未命名）"]
 GA --> RP
 end
```
...
```mermaid
flowchart TB
 subgraph G1["OBServer 强读 / 弱读副本判定决策链（§2.3.4）"]
 direction TB
 R1["读请求到达 ODP"] --> J1["判定一致性级别：hint READ_CONSISTENCY(WEAK) / session 变量 / 协议弱读标志"]
 J1 -->|"DML、SELECT ... FOR UPDATE、内部表"| S1["恒为强一致：select_leader_ = 1"]
 J1 -->|"显式弱读"| W1["弱一致：select_leader_ = 0"]
 S1 --> RT["执行期 ObDASLocationRouter::get_tablet_loc 按 select_leader_ 分叉"]
 W1 --> RT
 RT -->|"select_leader_ = 1，或重试遇 OB_NOT_MASTER"| L1["nonblock_get_leader：路由到 LS Leader 所在 OBServer"]
 RT -->|"select_leader_ = 0"| F1["nonblock_get_readable_replica；proxy_route_policy 优先转发 follower"]
 F1 -->|"follower 不可用"| FB["回退 leader；follower-only 策略则终止连接"]
 F1 -->|"受 follower latency threshold 约束"| FL["滞后超阈则不选该 follower"]
 end
```
...
flowchart TD
 ...
 RC -->|未命中/刷新| PD[(PD<br/>RegionInfo: Leader/Range/Epoch)]
 PD --> RC
 TASK --> LDR1[TiKV Region Leader A]
 TASK --> LDR2[TiKV Region Leader B 或可读 Follower]
 LDR1 --> AGG[TiDB 汇总结果]
 LDR2 --> AGG
 AGG --> C1
 end
 ...
 PR --> PC{本地缓存→全局缓存}
 PC -->|未命中| LSV[(OBServer 位置服务<br/>location_cache)]
 LSV -->|tablet→LS→leader| PC
 PC -->|强一致| L1[OBServer: LS Leader]
 PC -->|弱一致/LDC 就近| L2[OBServer: LS Leader 或 Follower]
 L1 --> R2[ODP 返回结果]
 L2 --> R2
 R2 --> C2
 end
...
```mermaid
flowchart TD
 START[请求发往缓存中的目标] --> CASE{命中什么异常}

 CASE -->|TiDB: EpochNotMatch/StaleCommand| E1[Region 已分裂/合并/迁移<br/>或命令过期]
 E1 --> E2[bo.Backoff BoRegionMiss 退避]
 E2 --> E3[InvalidateCachedRegion + 重新 buildCopTasks]
 E3 --> E4[向 PD 重新拉取 RegionInfo]
 E4 --> OK[重定位成功, 重发]

 CASE -->|TiDB: NotLeader| N1{响应是否带新 Leader}
 N1 -->|带| N2[据提示更新路由→新 Leader 重发]
 N1 -->|不带| N3[标记 NoLeader 失效→换 peer 重试<br/>小概率原 peer 已成 Leader, 再试 1 次]
 N2 --> OK
 N3 --> OK

 CASE -->|OceanBase: Leader 已切走| O1[OBServer 无法本地处理]
 O1 --> O2[反馈写入响应包回 ODP]
 O2 --> O3[ODP 强制刷新本地路由表 / reroute]
 O3 --> O4{强一致还是弱一致}
 O4 -->|强一致| O5[get_leader 重取 LS Leader<br/>缺失则重试到超时]
 O4 -->|弱一致| O6[follower-first/follower-only 选副本]
 O5 --> OK
 O6 --> OK

 CASE -->|控制面不可用| F1[PD 全挂 / sys tenant 不可写]
 F1 --> F2[缓存仍命中→已有连接可继续<br/>但新 Region/Leader 信息无法刷新]
 F2 --> F3[超时/退避耗尽→上抛错误给业务]
```
...
flowchart TB
 ...
 S3 -->|"多数派 handle_prepare_response（Promise）"| S4["RECONFIRM_FETCH_LOG"]
 S4 -->|"取多数派 max proposal_id / LSN，补齐/截断"| S5["RECONFIRMING"]
 S5 --> S6["START_WORKING"]
 S6 -->|"日志对齐完成"| S7["LEADER_ACTIVE"]
 S0 -->|"旧 Leader 失去身份"| S8["pending follower"]
 S8 -->|"等新 Leader 日志，判定 in-flight commit / rollback"| S0
 end
```
...
```mermaid
flowchart TD
 C[客户端写请求] --> R{Region cache 路由到 Region Leader}
 R --> L[Leader: raft-rs propose<br/>生成 log entry]
 L --> P[raftstore: WriteBatch 批量持久化<br/>cmd_batch=true]
 P --> A1[AppendEntries 复制]
 A1 --> F1[Follower1 持久化 + ack]
 A1 --> F2[Follower2 持久化 + ack]
 F1 --> Q{多数派 ack?<br/>match index 推进}
 F2 --> Q
 Q -- 是 --> CMT[commit:推进 commit index]
 CMT --> AP[apply 线程池<br/>ApplyDelegate 写 RocksDB]
 AP --> ACK[返回客户端成功]
 L -. Lease Read .-> RD[Leader Lease 内本地强一致读<br/>lease<=9s, 写续约]
```
...
```mermaid
flowchart TD
 AppC[事务请求] --> Ldr{LS Leader}
 Ldr --> Gen[1.事务引擎推进写入, 生成 redo 日志]
 Gen --> SL[2.ObLogHandler/submit_log 经 LogSlidingWindow 追加<br/>分配 LSN/SCN -- Accept]
 SL --> Rep[3.Paxos 按 LSN 复制]
 Rep --> Fa[Follower A 持久化 + ack_log]
 Rep --> Fb[Follower B 持久化 + ack_log]
 Fa --> Maj{get_majority_match_lsn<br/>多数派匹配?}
 Fb --> Maj
 Maj -- 是 --> Cmt[try_advance_committed_end_lsn<br/>推进 committed_end_lsn]
 Cmt --> Notify[推进事务状态为可提交]
 Cmt --> Replay[Followers replayservice 回放]
```
...
```mermaid
flowchart TD
 A[Leader 失联或网络分区] --> B{TiDB Region Raft Group}
 B --> B1[Followers 选举超时<br/>~10s 量级, tick=1s x election_tick=10]
 B1 --> B2[PreVote 预投票探测多数派]
 B2 -- 失败 --> B2x[不增 term, 不打断, 重试]
 B2 -- 成功 --> B3[RequestVote term+1]
 B3 --> B4[约束: 仅日志最新副本可当选<br/>Leader Completeness]
 B4 --> B5[新 Leader 提交 no-op 确立 commit 点]
 B5 --> B6[落后副本 catch-up / snapshot 补齐, 恢复写]
 B5 -. Hibernate Region 陷阱 .-> B7[故障节点上休眠 Region 不主动重选<br/>需请求触达 Follower 才唤醒, 见 §3.8]

 A --> G{OceanBase Log Stream / PALF}
 G --> G1[election 模块: lease 失效后按 priority 选 candidate]
 G1 --> G2[candidate 进入 LEADER_RECONFIRM]
 G2 --> G3[LogReconfirm.submit_prepare_log_<br/>Prepare new_proposal_id]
 G3 --> G4[多数派 handle_prepare_response<br/>回 max accept proposal_id / LSN]
 G4 --> G5[取多数派 max<br/>恢复/截断未确认日志, 已被多数派接受者必不丢]
 G5 --> G6[转 LEADER_ACTIVE, 恢复写]
 G6 -. 旧 Leader 复活 .-> G7[先进 pending follower 等新 Leader 日志<br/>判定 in-flight 日志 commit/rollback]
```
...
```mermaid
flowchart TB
 subgraph SST["SSTable（磁盘基线 / 落盘增量数据）"]
 direction TB
 SSTD["职责：含一个或多个 macroblock"]
 subgraph MACRO["Macroblock（固定 2MB，写出与 GC 基本单位）"]
 direction TB
 MACROD["职责：索引 / 校验 / 压缩 / Bloom filter / merge 信息 / 事务版本边界"]
 subgraph MICRO["Microblock（变长，读的基本 I/O 单位，默认 16KB）"]
 direction TB
 MICROD["职责：按列 encoding / 压缩（bit-packing / 字典 / RLE / delta）"]
 ROWS["一行或多行数据"]
 MICROD ~~~ ROWS
 end
 MACROD ~~~ MICRO
...
```mermaid
flowchart TD
 subgraph TiKV["TiKV 写入正常路径"]
 direction TB
 A1[TiDB SQL 层编码 KV mutation] --> A2[定位 Region leader]
 A2 --> A3[Raft propose 复制日志]
 A3 --> A3R[Raft Engine: 顺序追加 active log file]
 A3 --> A4{多数派 committed?}
 A4 -->|是| A5[apply 线程: RocksDB WriteBatch]
 A5 --> A9[按 CF 分流: 255B 阈值]
 A9 --> A9L[lock CF: primary lock/悲观锁]
 A9 --> A9D[default CF: value + start_ts]
 A9 --> A9W[write CF: commit record + commit_ts]
 A5 --> A6[写共享 WAL + MemTable skiplist]
 A6 --> A7[flush 到 SSTable]
 A7 --> A8[leveled/universal compaction 本地自治]
 end
 subgraph OB["OceanBase 写入正常路径"]
 direction TB
 B1[SQL/事务层生成行变更] --> B1L[定位 Tablet 所在 LS / leader]
 B1L --> B2[副本同步 PALF 日志 详见第3章]
 B2 --> B3[写 MemTable: ObKeyBtree + hash]
 B3 --> B4{MemTable 达阈值?}
 B4 -->|freeze| B4F[active -> frozen MemTable]
 B4F --> B5[Mini Merge: 仅刷 memtable -> Mini SSTable]
 B5 --> B6[Minor Merge: 多 mini -> Minor SSTable]
 B6 --> B7[读: 基线 Major SSTable ⊕ 增量 on-the-fly 归并]
 B7 -.RootService 选全局版本号.-> B8[Major Freeze]
 B8 --> B9[全副本 Major Merge -> 新基线 + checksum 校验]
 B9 --> B10[SSTable -> Macroblock -> Microblock]
 end
...
```mermaid
flowchart TD
 subgraph TiKVF["TiKV 故障 / 异常路径"]
 direction TB
 TA[节点磁盘损坏] --> TB[单 TiKV RocksDB 实例不可读 -> 该副本失联]
 TB --> TC[Raft 多副本仍可服务 PD 调度补副本 详见第3章]
 TD[Region leader 故障] --> TE[Raft 重新选主]
 TE --> TF[新 leader 读 Raft Engine / 本地状态机]
 TF --> TG[继续 apply 到 RocksDB CF]
 TG --> TH{RocksDB write stall?}
 TH -->|L0 堆积/pending compaction bytes 超阈| TI[前台写/apply 尾延迟飙升 -> Region 写入抖动]
 TH -->|否| TJ[服务恢复]
 end
 subgraph OBF["OceanBase 故障 / 异常路径"]
 direction TB
 OA[Major Merge 抖动] --> OB1[集群级合并: 读全部基线 + 写新基线]
 OB1 --> OC[I/O/CPU 峰值 -> 业务延迟周期性上升]
 OC --> OD[缓解: incremental/progressive/parallel/rotation merge 分摊]
 OE[RootService 不可用] --> OF[major freeze 调度受阻 -> 无法推进新基线]
 OF --> OG[增量 MemTable/Mini/Minor 持续堆积]
 OG --> OH[读路径归并层数变多 -> 读放大与内存压力上升]
 OI{SCN range / snapshot / checksum 是否正常?} -->|异常| OJ[某副本基线与全局版本不一致 -> 报警/阻断该版本]
 OI -->|正常| OK[生成新的 SSTable / Major baseline]
 end
...
```mermaid
flowchart TD
 SQL["SQL: SELECT/INSERT 关系操作"] --> TiDBS["TiDB Server<br/>SQL 解析/优化"]
 TiDBS -->|"关系→KV 编码<br/>pkg/tablecodec"| ENC["行 key=t{tid}_r{handle}<br/>索引 key=t{tid}_i{iid}{vals}<br/>行格式 v2 CodecVer=128"]
 ENC --> AP{"访问路径"}
 AP -->|"聚簇主键"| GET["Get 行 key"]
 AP -->|"二级索引"| IDX["Index Scan 索引 key range"]
 IDX --> COV{"覆盖索引？"}
 COV -->|"是"| RET["返回索引列"]
 COV -->|"否"| BACK["按 handle 回表读行 key"]
 GET --> MVCC["TiKV MVCC<br/>key.append_ts → 三 CF (lock/write/default)"]
 BACK --> MVCC
 MVCC --> RDB["RocksDB (LSM)<br/>WAL→MemTable→SSTable(≤6层)"]
 RDB -.->|"Raft log 独立通道"| RE["Raft Engine (v6.1+)"]

 SQL2["SQL / OBKV (OceanBase)"] --> OBS["OBServer<br/>SQL + 存储一体"]
 OBS -->|"关系原生，无不透明KV层<br/>按 Tablet/主键范围定位"| BSS["blocksstable<br/>ob_row_writer/ob_datum_row"]
 BSS --> MEM["MemTable(增量，行操作链)"]
 MEM -->|"dump/minor compaction"| SST["SSTable(基线)<br/>宏块2MB + 微块PAX编码"]
 SST -.->|"major compaction<br/>基线+增量融合"| SST
 BSS -.->|"共识日志独立"| PALF["PALF (Paxos log)"]
```
...
```mermaid
flowchart TD
 W["写请求"] --> REP["复制式 WAL / Paxos-backed append-only log + 事务状态推进"]
 REP --> FAIL{"卡在哪一层？"}
 FAIL -->|"A. Leader/副本切换"| ROUTE["客户端路由缓存失效"]
 ROUTE --> RFR["向 PD/RS 刷新 Region/Tablet 位置"]
 RFR --> RETRY["backoff 重试 prewrite/commit/read 到新 Leader"]
 FAIL -->|"B. compaction 落后 (LSM 共性)"| PEND["pending compaction bytes ↑ / L0 文件堆积"]
 PEND --> LIM{"超 soft/hard limit?"}
 LIM -->|"是"| STALL["TiKV scheduler 流控(discard ratio↑)<br/>OB 分级转储+资源隔离<br/>→ 前台写延迟尖刺"]
 LIM -->|"否"| FLUSH["flush SSTable，稍后 compact"]
 FAIL -->|"C. 节点 crash"| RECO["MemTable 未落盘部分<br/>从 WAL(单机)/共识 log(副本)重放恢复"]
 FAIL -->|"D. 事务未提交"| LOCK["lock CF 中 prewrite/悲观锁可见"]
 LOCK --> RESOLVE["resolve lock / rollback / commit 恢复"]
```
...
```mermaid
flowchart TB
 subgraph OBS["OBServer 单进程内模块组成"]
 direction TB
 SQL["SQL 引擎 ObSql"] ~~~ SESS["会话管理 ObSQLSessionMgr"]
 SESS ~~~ MT["多租户管理 ObMultiTenant"]
 MT ~~~ SCH["schema 服务 多版本"]
 SCH ~~~ TXN["事务服务 ObTransService"]
 TXN ~~~ PALF["PALF 共识日志 ObLogService"]
 PALF ~~~ LS["Log Stream 存储 ObLSService"]
 LS ~~~ STO["存储环境 MemTable/SSTable"]
 STO ~~~ RPC["RPC proxy / location service"]
 end
 subgraph TIDB["对照:TiDB 多组件分离"]
 direction TB
 TS["TiDB Server 无状态计算"] ~~~ TK["TiKV 行数据"]
 TK ~~~ PD["PD 元数据/TSO"]
 end
 OBS ~~~ TIDB
```
...
```mermaid
flowchart TB
 subgraph TiDB["TiDB:无状态 SQL 层"]
 direction TB
 C1[客户端] --> LB[TiProxy / LVS / HAProxy]
 LB --> T1[TiDB Server A 无状态]
 LB --> T2[TiDB Server B 无状态]
 T1 -. 拉取并缓存 .-> PD[(PD: TSO/元数据)]
 T1 -. schema 缓存 .-> KVm[(TiKV: schema 元数据)]
 T1 -->|跨网络 RPC| KV[(TiKV: 行数据)]
 T2 -->|跨网络 RPC| KV
 T1 -. 列存可选 .-> TF[(TiFlash)]
 end

 subgraph OB["OceanBase:一体化 OBServer"]
 direction TB
 C2[客户端] --> ODP[OBProxy / ODP 连接+路由]
 ODP -->|按 partition 路由| O1[OBServer 1 进程内]
 ODP --> O2[OBServer 2 进程内]
 O1 --> S1[SQL 引擎]
 S1 -.函数调用.-> X1[事务引擎]
 X1 -.函数调用.-> ST1[存储 MemTable/SSTable]
 X1 -.函数调用.-> P1[PALF 复制式 WAL]
 P1 ==Paxos 多数派==> O2
 O1 -.函数调用.-> RS[RootService/schema/location]
 end
...
```mermaid
flowchart TB
 A[TiDB Server A 崩溃或滚动升级] --> B{有 TiProxy?}
 B -->|是| C[SHOW SESSION_STATES 导出会话]
 C --> D{会话可迁移?}
 D -->|无本地临时表/表锁/advisory lock 且事务已结束游标已取尽| E[SET SESSION_STATES 到 B 实例 客户端无感]
 D -->|含本地临时表/表锁/advisory lock 直接拒绝| F[ErrCannotMigrateSession session:8146]
 D -->|含活跃事务/未读游标 须等待| F2[等事务结束或结果取尽再迁]
 B -->|否或意外宕机| G[连接断开 客户端重连到 B 实例]
 F2 --> E
 E --> H[新实例 plan cache 冷启动 miss 编译开销]
 G --> H

 I[TiDB 与 etcd 会话断开] --> J[集群视为 down 删除 schema version key]
 J --> K[停止 schema validator 禁止 DML]
 K --> L[重连后 mustReload 重建 schema 缓存 恢复服务]

 M[OBServer 宕机] --> N[ODP 检测并刷新路由]
 N --> O[其 Log Stream Follower 经 Paxos 选举 + LogReconfirm 重建 Leader]
 O --> P[RPO=0 数据不丢 但需读日志重放 并涉租户/LS/事务参与者恢复 非秒级切流量]
```
...
```mermaid
flowchart TB
 subgraph CLASSIC["TiDB classic OLTP(shared-nothing)"]
 direction TB
 C1["Client"] --> LB["TiProxy / LB"]
 LB --> TIDB["TiDB Server<br/>SQL parse/optimize/execute(无状态)"]
 TIDB --> PD["PD<br/>TSO / metadata / scheduling"]
 TIDB --> TIKV["TiKV Region leader<br/>Raft + RocksDB"]
 TIKV --> RF["Raft followers x2"]
 TIKV --> LSM["Local RocksDB<br/>WAL/SSTable/cache/compaction"]
 end

 subgraph TIFLASH["TiFlash disaggregated AP"]
 direction TB
 TIDB --> TC["TiFlash Compute Node<br/>无状态查询 · 秒级伸缩"]
 TC --> TW["TiFlash Write Node<br/>snapshot / 最新数据 / 列转换"]
 TW --> S3["S3 / S3 兼容对象存储<br/>列式快照(权威副本)"]
 TC --> S3
 TC --> CC["本地 NVMe remote-data cache"]
 TIKV -. "Raft 日志" .-> TW
 TIDB -. "consistent_hash 路由提命中" .-> TC
 end

 subgraph OBSS["OceanBase shared-storage(Bacchus)"]
 direction TB
 C2["Client"] --> ODP["OBProxy / ODP"]
 ODP --> RW["OBServer RW node"]
 ODP --> RO["OBServer RO node"]
 RW --> LSVC["共享日志服务<br/>PALF 多副本共识(RPO=0)"]
 RO --> LSVC
 RW --> SSW["SSWriter<br/>增量/基线上传"]
 SSW --> OBJ["共享对象存储<br/>SSTable 主存"]
 RW --> HC["本地三级缓存<br/>memory micro / 本地持久 / 分布式 macro"]
 RO --> HC
 RO --> OBJ
 end
...
flowchart TD
 ...
 B -->|TiDB classic| C["TiKV Region leader 失效"]
 C --> D["Raft follower 选新 leader"]
 D --> E["TiDB RegionCache epoch/leader 刷新"]
 E --> F["重试或返回可解释错误<br/>(多副本本地数据继续服务)"]

 B -->|TiFlash disagg| G["Compute Node 崩溃<br/>(无持久状态可丢失)"]
 G --> H["路由层一致性哈希重映射<br/>新节点秒级拉起"]
 H --> I["连 Write Node / S3 按需加载工作集<br/>(无需源节点拷贝)"]
 I --> J["冷缓存期 cache miss<br/>→ 读穿透对象存储(数十 ms 量级)→ 尾延迟抖动"]

 B -->|对象存储抖动| K["对象存储写入超时<br/>_object_storage_io_timeout(默认 20s)"]
 K --> L{"重试 / 降级"}
 L -->|成功| M["恢复正常上传"]
 L -->|持续失败| N["本地写缓存堆积 → 背压前台写<br/>OB 本地云盘第一保障兜底"]

 B -->|OceanBase SS| P["RW/RO 或共享日志服务异常"]
 P --> Q["RW 切换或 RO 重建本地 hot state"]
 Q --> R["PALF 重选主 + LogReconfirm(详见第 3 章)<br/>多副本维持日志持久性 RPO=0"]
 R --> S["从对象存储 + log replay 重建热数据<br/>单副本数据层 RTO 分钟级"]

 B -->|后台任务| T["compaction/DDL/import 资源升高"]
 T --> U["classic 路径可能争抢本地 CPU/IO"]
 T --> V["TiDB X / Bacchus 倾向后台服务化隔离"]
```
...
```mermaid
flowchart TB
 subgraph TiDB["TiDB OLTP 写事务正常路径"]
 direction TB
 A1[客户端 SQL] --> A2[TiDB Server: parse/compile/execute]
 A2 --> A3[取 TSO: PD leader<br/>客户端批量攒批]
 A3 --> A4[定位 Region + 悲观锁加锁<br/>Region Raft 多数派]
 A4 --> A5[prewrite: primary + secondary<br/>各 Region Raft 多数派]
 A5 --> A6[commit / async commit<br/>取 commit_ts 再多数派]
 A6 --> A7[Raft apply 落 RocksDB/MVCC]
 A7 --> A8[返回客户端]
 A4 -. Raft Learner 异步复制 .-> A9[TiFlash MPP<br/>计划选列副本时承担 AP]
 end
 subgraph OB["OceanBase OLTP 写事务正常路径"]
 direction TB
 B1[客户端 SQL] --> B2[ODP 路由到 LS leader]
 B2 --> B3{单 LS 事务?}
 B3 -->|是| B4[免 GTS RPC<br/>单机提交协议]
 B3 -->|否| B5[取 GTS: 本地缓存命中?<br/>否则异步 RPC 刷新]
 B5 --> B6[树形 2PC: prepare<br/>各 LS PALF 多数派]
 B4 --> B7[PALF 多数派持久化<br/>+ replay/apply/checkpoint]
 B6 --> B7
 B7 --> B8[返回客户端]
 B2 -. AP 路径 .-> B9[列存 + 向量化执行]
 end
...
```mermaid
flowchart TD
 subgraph TiDBF["TiDB 异常路径"]
 direction TB
 T0[云盘 IO 抖动] --> T1[Raft commit / fsync 延迟↑]
 T1 -->|8.5 优化| T2[leader 提前 apply<br/>已提交未持久化日志<br/>P99/999 最多降约 98%]
 T1 --> T3[L0 文件堆积 / pending bytes↑]
 T3 -->|超 level0_slowdown=20| T4[write slowdown/stop 或 flow control]
 T4 --> T9[写延迟尖刺 / 尾延迟上升]
 T5[TiKV 节点宕 / Region leader 失效] --> T6[PD 检测 + Raft 重选<br/>PreVote/Lease]
 T6 --> T7[RegionCache 失效<br/>RegionError 驱动客户端刷新路由]
 T7 --> T8[新 leader 提供服务]
 end
 subgraph OBF["OceanBase 异常路径"]
 direction TB
 O0[每日 02:00 major merge] --> O1[全量重写 SSTable<br/>IO/CPU 抖动 + 删历史数据]
 O1 --> O2[延迟毛刺 凌晨可见]
 O0 -.错峰.-> O3[major_freeze_duty_time 配置]
 O4[memstore 达 throttling=60%] --> O5[主动写限速 写延迟↑]
 O6[OBServer 宕 / LS leader 失效] --> O7[仅该 LS 快速重选<br/>其它 LS 继续服务]
 O7 --> O8[Paxos 选举 + LogReconfirm<br/>日志追赶 见第 3、10 章]
 O8 --> O9[ODP location 刷新 → 恢复]
 end
...
```mermaid
flowchart TB
 subgraph G1["TiDB:表 → Region → TiKV 节点"]
 direction TB
 T1["用户表 orders / order_detail / payment"]
 T2["各表编码为连续 KV key range,自动横切 Region"]
 T3["Region 分布于 TiKV 节点(PD 自然均衡)"]
 T4["无内核级共置约束:不保证相关表行落同一 Region / 节点"]
 T1 --> T2 --> T3 --> T4
 end
 subgraph G2["OceanBase:表 → Partition → Tablet → LS"]
 direction TB
 O1["用户表 orders / order_detail / payment"]
 O2["按 PARTITION BY 水平拆为多个 Partition"]
 O3["每个物理分区一一对应一个 Tablet"]
 O4["Tablet 归属 Log Stream(LS)"]
 O5["Table Group(SHARDING=PARTITION):同订单分区共置同一 LS"]
 O1 --> O2 --> O3 --> O4 --> O5
 end
 G1 ~~~ G2
```
...
```mermaid
flowchart TB
 App["应用<br/>看到一张逻辑表 orders"]

 subgraph TiDB["TiDB:逻辑表到 Region"]
 direction TB
 T1["TiDB Server<br/>SQL/计划/KV 编码<br/>t{tableID}_r{handle}"]
 T2["AUTO_RANDOM 填随机 shard bit<br/>打散 handle 高位"]
 T3["client-go RegionCache + PD<br/>LocateKey 定位 Region/Leader"]
 T4["TiKV: Region(默认 256MiB)<br/>size/keys 超阈 → split-checker 拆分"]
 T5["PD 后续调度副本/Leader 均衡<br/>split / scatter"]
 T1 --> T2 --> T3 --> T4 --> T5
 end

 subgraph OB["OceanBase:Partition 到 Tablet/LS"]
 direction TB
 O1["ODP/OBProxy<br/>SQL 解析 + 分区裁剪(pruning)"]
 O2["按 PARTITION BY(HASH/KEY/RANGE)<br/>Partition key → Partition → Tablet"]
 O3["Tablet 所属 Log Stream Leader<br/>PALF/Paxos 复制 redo"]
 O4["Table Group 共置:<br/>order 与 order_detail 同 LS<br/>→ 本地事务/partition-wise join"]
 O5["RootService 调度<br/>Tablet/LS 均衡与 transfer"]
 O1 --> O2 --> O3 --> O4 --> O5
 end
...
```mermaid
flowchart TD
 W["业务写订单/明细/支付"] --> Q{"底层参与者是否稳定?"}

 Q -->|"TiDB:自增 PK 未用 AUTO_RANDOM"| TA["末尾 Region 热点<br/>单 Region 写放大、排队"]
 Q -->|"TiDB:Region split/scatter 未完成"| TB["RegionError(EpochNotMatch)<br/>RegionCache 失效 → 按 epoch/PD 重定位"]
 Q -->|"TiDB:跨 Region 事务 prewrite 后 Leader 宕"| TC["残留 primary lock<br/>惰性 ResolveLock 清理(第 10 章)"]
 Q -->|"TiDB:全局索引写入跨分片"| TD["升级为分布式事务<br/>写放大、长尾延迟(第 15 章)"]

 Q -->|"OB:分区键选错(自增列 RANGE)"| OA["末尾分区 Tablet 热点<br/>负载倾斜"]
 Q -->|"OB:Tablet transfer / LS 切主"| OB1["Tablet-to-LS 映射更新<br/>路由表失效需刷新(第 2 章)"]
 Q -->|"OB:跨 LS 事务 prepare 后 Leader 宕"| OC["新 Leader 重放 2PC 日志<br/>+ orphan 重建(第 10 章)"]
 Q -->|"OB:Table Group 共置失效 / 全局唯一索引写"| OD["跨 Tablet 分布式事务<br/>写放大、远程执行增加"]

 TA --> R["共同结论:消除的是应用层分片管理<br/>未消除分布式物理代价"]
 TB --> R
 TC --> R
 TD --> R
 OA --> R
...
```mermaid
flowchart TB
 subgraph G1["OceanBase 树形 2PC 协调者拓扑（角色:ROOT / INTERNAL / LEAF）"]
 direction TB
 ROOT["ROOT(第一个参与者 LS / 协调者)"] -->|"prepare / commit 请求"| INT1["INTERNAL(中间节点 LS:对子为协调者,对父为参与者)"]
 ROOT -->|"prepare / commit 请求"| LEAF1["LEAF(叶子参与者 LS)"]
 INT1 -->|"prepare / commit 请求"| LEAF2["LEAF(叶子参与者 LS)"]
 INT1 -->|"prepare / commit 请求"| LEAF3["LEAF(叶子参与者 LS)"]
 end
```
...
sequenceDiagram
 ...
 participant TiDB as TiDB Server(client-go)
 participant PD as PD(TSO)
 participant R1 as Region1 Leader(primary)
 participant R2 as Region2 Leader(secondary)
 App->>TiDB: BEGIN / DML / COMMIT
 TiDB->>PD: 取 start_ts
 PD-->>TiDB: start_ts
 Note over TiDB: 选 primary key,按 Region 分组写集到 R1/R2
 par 并发 Prewrite
 TiDB->>R1: Prewrite(primary, secondaries列表)
 R1->>R1: 写 lock/default CF → Raft 多数派落盘
 R1-->>TiDB: ok(min_commit_ts1)
 and
 TiDB->>R2: Prewrite(secondary, 指向primary)
 R2->>R2: 写 lock/default CF → Raft 多数派落盘
 R2-->>TiDB: ok(min_commit_ts2)
 end
 Note over TiDB: commit_ts = max(min_commit_ts1, min_commit_ts2)
 TiDB-->>App: 返回成功(async commit:无需等 commit 落盘)
 par 异步 Commit
 TiDB->>R1: Commit(primary, commit_ts) → 写 write CF/释放锁 → Raft 落盘
 and
 TiDB->>R2: Commit(secondary, commit_ts) → 写 write CF/释放锁 → Raft 落盘
 end
```
...
sequenceDiagram
 participant App as 应用
 participant LS1 as LS1 Leader(ROOT=第一个参与者/协调者)
 participant LS2 as LS2 Leader(LEAF 参与者)
 participant GTS as 租户 GTS
 App->>LS1: DML(redo 写入 LS1)
 App->>LS2: DML(redo 写入 LS2)
 App->>LS1: COMMIT
 Note over LS1: 进入 PREPARE
 par Prepare(redo+commit_info 多数派)
 LS1->>LS1: 写 PREPARE 日志 → Paxos 多数派
 and
 LS1->>LS2: prepare 请求
 LS2->>LS2: 写 PREPARE 日志 → Paxos 多数派
 LS2-->>LS1: prepare-ok(本地 prepare SCN)
 end
 LS1->>GTS: 取 commit 版本(SCN)
 GTS-->>LS1: commit SCN
 Note over LS1: 全部 prepare-ok → PRE_COMMIT
 LS1-->>App: 返回成功(prepare ACK 后即返回)
 par Commit / Clear(异步)
 LS1->>LS1: 写 COMMIT 日志 → 释放锁
 and
 LS1->>LS2: commit 请求 → LS2 写 COMMIT → CLEAR
 end
```
...
```mermaid
flowchart TD
 A[Reader 以 ts 读某 key] --> B{该 key 有锁?}
 B -- 无 --> Z[正常读 write/default CF]
 B -- 有锁 --> C[沿 secondary 指向<br/>向 primary 所在 Region 发 CheckTxnStatus]
 C --> D{primary 状态?}
 D -- Committed --> E[roll forward<br/>按 commit_ts 提交该 secondary]
 D -- RolledBack --> F[回滚该 secondary 锁]
 D -- 锁仍在且未过 TTL --> G[事务可能还活着<br/>→ 等待/退避/重试]
 D -- TtlExpire<br/>lock.ts.physical+ttl<current --> H[判定事务死亡 → 清锁]
 E --> I[ResolveLock 批量处理同事务<br/>全部 secondary 并唤醒阻塞事务]
 F --> I
 H --> I
 G --> A
 I --> Z
...
```mermaid
flowchart TD
 A[跨 LS 事务进行中] --> B[LS-X Leader 崩溃 / 发生 leader transfer]
 B --> C[新 Leader 上任 → 重放 PALF 中本 LS 的 2PC redo 日志]
 C --> D{读到的最新 2PC 日志态?}
 D -- PREPARE 但无 COMMIT/ABORT --> E[进入未决态<br/>向上游协调者 LS 发 orphan/查询<br/>handle_orphan_2pc_*]
 D -- PRE_COMMIT --> F[precommit 共识已达成<br/>可在本机提交]
 E --> G{上游全局结局?}
 G -- 已 COMMIT --> H[重放至 COMMIT → CLEAR]
 G -- 已 ABORT --> I[回滚至 ABORT]
 J[协调者第一个参与者崩溃] --> K[新 Leader 查询全部参与者状态<br/>coordinator-less 无全局协调者日志<br/>靠参与者状态汇总重建结局]
 F --> L[继续 commit / clear 或返回可判定状态]
 H --> L
 I --> L
 K --> L
```
...
```mermaid
flowchart TB
 subgraph CF["TiKV 三列族物理布局"]
 direction TB
 LOCK["CF_LOCK：user_key 到 lock_info"]
 WRITE["CF_WRITE：（user_key, commit_ts）到 write_info（内含 start_ts）"]
 DEFAULT["CF_DEFAULT：（user_key, start_ts）到 value"]
 LOCK ~~~ WRITE ~~~ DEFAULT
 end

 subgraph READ["一次快照读跨 CF 解析"]
 direction TB
 S1["检查 lock CF：查 lock_ts <= ts 的锁"]
 S2{"有且非本事务？"}
 S3["命中未决事务，走锁冲突处理"]
 S4["读 write CF：seek（user_key, ts），取 commit_ts <= ts 最新提交记录，读出 start_ts"]
 S5{"WriteType？"}
 S6["内联 short_value：直接返回值"]
 S7["按指针回 default CF：取（user_key, start_ts）实际值"]
 S8["Delete：返回空"]
 S9["Lock / Rollback：继续向更旧版本迭代"]
 S1 --> S2
 S2 -->|"有"| S3
 S2 -->|"无"| S4
 S4 --> S5
 S5 -->|"Put 且 short_value"| S6
 S5 -->|"Put 无 short_value"| S7
 S5 -->|"Delete"| S8
 S5 -->|"Lock / Rollback"| S9
 end
...
```mermaid
flowchart TB
 subgraph TiDB["TiDB / TiKV 快照读或 stale read"]
 direction TB
 TSQL["TiDB SQL 执行器"] --> TTSO["从 PD/TSO 申请 read_ts/start_ts"]
 TTSO --> TRoute["定位 Region 与目标 peer"]
 TRoute --> TSafe{"stale read 且 ts <= safe-ts?"}
 TSafe -- "是" --> TFollower["读本地 peer 快照"]
 TSafe -- "否/最新读" --> TLeader["Leader 或 ReadIndex 验证读"]
 TFollower --> TLock{"CF_LOCK / 内存悲观锁<br/>有 lock_ts<=ts 的锁?"}
 TLeader --> TLock
 TLock -- "有且非本事务" --> TResolve["Lock Resolver 回查 primary lock"]
 TLock -- "无" --> TWrite["CF_WRITE seek: commit_ts<=read_ts<br/>取最新提交记录的 start_ts"]
 TWrite --> TType{"WriteType?"}
 TType -- "Put 且 short_value" --> TResult["返回内联 value"]
 TType -- "Put 无 short_value" --> TData["CF_DEFAULT 取 (key, start_ts)"]
 TType -- "Delete" --> TEmpty["返回空"]
 TData --> TResult
 end

 subgraph OB["OceanBase 强一致读 / 弱读"]
 direction TB
 OSQL["OBServer SQL 执行器"] --> OCons["从协议/hint/session 解析 STRONG/WEAK"]
 OCons --> OSnap{"一致性类型?"}
 OSnap -- "STRONG" --> OCurrent["取当前读快照 / 路由 Leader"]
 OSnap -- "WEAK" --> OWeak["取 bounded-staleness 弱读快照<br/>优先 Follower(weak read ts)"]
 OCurrent --> OStore["读 MemTable: 遍历 ObMvccRow trans node 链"]
 OWeak --> OStore
 OStore --> OMvcc["回查 tx_table: 按 state/commit_version 判可见性"]
 OMvcc --> OMerge["读 SSTable: 按 trans_version 过滤隐藏列<br/>融合 MemTable/minor/major 多版本"]
 OMerge --> OResult["返回可见的已提交行"]
 end
...
```mermaid
flowchart TB
 subgraph TiDBFault["TiDB / TiKV 异常读路径"]
 direction TB
 A["读命中锁或路由异常"] --> B{"哪种条件?"}
 B -- "read_ts 前存在锁" --> C["Lock Resolver 回查 primary lock / 事务状态"]
 C --> D{"事务已提交?"}
 D -- "是" --> E["补写 secondary commit, 暴露 commit_ts, 重试读"]
 D -- "否/已超时" --> F["写 Rollback 或清锁，重试读"]
 B -- "Region leader/epoch 变化" --> G["返回 stale command / epoch not match"]
 G --> H["TiDB 刷新 Region cache 后重试"]
 B -- "GC safepoint 越过 read_ts" --> I["读可能因 GC life time 短于事务时长而失败"]
 I --> J["调大 GC lifetime 或缩短长事务<br/>v6.1+ tidb_gc_max_wait_time 强推 safepoint"]
 end

 subgraph OBFault["OceanBase 异常读路径"]
 direction TB
 K["读请求"] --> L{"一致性与副本状态?"}
 L -- "弱读副本落后 > max_stale_time" --> M["该副本不可读，重试其他有效副本"]
 L -- "强读时 Leader 不可用" --> N["Paxos 重选 Leader / LogReconfirm<br/>等新 Leader 或刷新路由"]
 L -- "minor SSTable 堆积 / merge 落后" --> O["读时合并触及更多版本/块，读放大上升"]
 L -- "长事务持旧快照 / 热点行长链" --> P["版本回收或 merge 边界被推迟<br/>trans node 链 > 500 建 ObMvccRowIndex"]
 M --> Q["按需重取/校验快照后返回"]
 N --> Q
 O --> Q
 P --> Q
...
```mermaid
flowchart TB
 subgraph TiDB["TiDB 正常路径（MPP / TiFlash）"]
 direction TB
 A1[SQL 文本] --> A2[Parser AST]
 A2 --> A3["逻辑优化 logicalOptimize<br/>optRuleList: 谓词/聚合/TopN 下推 + Join 重排"]
 A3 --> A4["物理优化 physicalOptimize<br/>FindBestTask 选最小代价物理计划"]
 A4 --> A5{命中 TiFlash<br/>且适合并行?}
 A5 -->|是| A6["切 Fragment<br/>ExchangeSender/Receiver"]
 A6 --> A7["MPPGather 下发<br/>Broadcast/Hash/PassThrough"]
 A7 --> A8["TiFlash 节点并行 join+agg"]
 A8 --> A9[回传 TiDB 汇总]
 A5 -->|否| A10["Coprocessor DAG 下推 TiKV<br/>就地过滤/聚合/TopN"]
 A10 --> A9
 end
 subgraph OB["OceanBase 正常路径（PX）"]
 direction TB
 B1[SQL 文本] --> B2["OBProxy/ODP 路由"]
 B2 --> B3["Parser/Resolver/Transformer"]
 B3 --> B4["ObOptimizer CBO<br/>分区裁剪 + DistAlgo 选分发"]
 B4 --> B5{计划态判定<br/>OB_PHY_PLAN_*}
 B5 -->|local/remote| B6[本地/单节点 RPC 执行]
 B5 -->|distributed| B7["QC 调度 DFO<br/>切分 Job/Task"]
 B7 --> B8["各节点拉起 SQC<br/>申请线程 + 构建上下文"]
 B8 --> B9["GI 切 granule 并行扫描<br/>bloom filter 运行期裁剪"]
 B9 --> B10["EXCHANGE 重分布<br/>broadcast/hash-hash/partition-wise"]
 B10 --> B11[QC 汇总回客户端]
 end
...
```mermaid
flowchart TD
 S["SQL: 统计信息过时 / 数据倾斜"] --> EST{"优化器估计偏差"}
 EST -->|TiDB 估小： 应 shuffle 误选 broadcast<br/>或 IndexJoin 内表估值爆炸| T1["大表广播到每个 TiFlash 节点<br/>或 NLJ 对内表反复扫描"]
 EST -->|OB 估小： partition-wise 误选 broadcast<br/>或 hash-hash 遇严重倾斜| O1["高频值集中到单个 PX worker<br/>单 DFO 线程成为长尾"]
 EST -->|漏掉分区裁剪 / MPP-PX 资源紧张| X1["额外 Region/Tablet 扫描<br/>exchange 积压 / 内存压力"]
 T1 --> T2["gRPC 回传快于消费 → cop OOM<br/>查询延迟从毫秒涨到分钟"]
 O1 --> O2["worker 内存超租户 SQL memory 配额<br/>→ 落盘或报错"]
 X1 --> Y["spill / 重试 / 慢查询"]
 T2 --> OBS["观测： EXPLAIN ANALYZE / SQL 执行诊断 / 日志"]
 O2 --> OBS
 Y --> OBS
 OBS --> T3["TiDB 兜底： ANALYZE 重建统计 /<br/>关闭过时伪统计 / 强制 hint(BROADCAST_JOIN 等)"]
 OBS --> O3["OB 兜底： 运行期自适应(滑动窗口停用失效 join filter /<br/>自适应 HASH GROUP BY / 自适应混合 hash shuffle 抗倾斜)+ hint"]
```
...
```mermaid
flowchart TB
 subgraph G1["sys tenant 控制面"]
 direction TB
 SYS["sys tenant"] --> RS["RootService"]
 RS --> UM["ObUnitManager"]
 end
 subgraph G2["资源层级"]
 direction TB
 UNIT["UNIT 最小资源单位"] --> RP["Resource Pool"]
 RP --> TEN["Tenant"]
 end
 subgraph G3["内部表与 GTS 映射"]
 direction TB
 ACT["__all_core_table TID=1 集群首表"] --> GS["sys tenant GTS provider"]
 DUM["__all_dummy TID=135"] --> GU["用户租户 GTS provider"]
 end
 G1 ~~~ G2
 G2 ~~~ G3
 UM --> UNIT
...
```mermaid
flowchart TD
 APP[客户端 SQL] --> TIDB[TiDB Server]
 TIDB -->|1. 取 start_ts 一次 RTT| TSO[PD Leader: GlobalTSOAllocator]
 TSO -->|内存窗内递增, 窗上界已持久化| ETCD[(内嵌 etcd / Raft)]
 TIDB -->|2. schema cache 校验| DOM[Domain / InfoSchema / SchemaValidator]
 TIDB -->|3. 查 Region 路由 region cache| PDMETA[PD: Region/Store 元数据]
 TIDB -->|4. 读写 KV| TIKV[TiKV Region Leader]
 TIKV -.->|Region/Store 心跳上报| PDMETA
 PDMETA -.->|调度算子搭载心跳响应| TIKV
 OWNER[DDL owner] -->|推进 schema_version| ETCD2[(etcd: global_schema_version)]
 OWNER -->|owner election via etcd| ETCD
 ETCD2 -->|Watch 通知| DOM
 TIDB -->|确认新版本| ETCD2
 subgraph 控制面
 direction TB
...
```mermaid
flowchart TD
 APP2[客户端 SQL] --> ODP[ODP / OBProxy]
 ODP -->|从 sys tenant 取 RS list/分区/zone/租户信息| SYS[sys tenant: RootService]
 ODP -->|路由到主副本所在 OBServer| OBS[OBServer SQL/Txn: LS Leader]
 OBS -->|取 commit/snapshot 版本| GTS[租户 GTS Leader: ObGtsSource]
 GTS -->|本地 cache + Paxos 复制| GTSP[(tenant internal table Leader 三副本)]
 OBS -->|读写 LS 副本| LS[Log Stream / LS Leader]
 SYS -->|schema guard 需 sys+租户双版本| SCHEMA[ObMultiVersionSchemaService]
 SCHEMA --> OBS
 SYS -.->|Unit/Locality/PrimaryZone 调度| OBS
 subgraph 控制面 sys tenant
 direction TB
 SYS
 SCHEMA
 end
...
```mermaid
flowchart TD
 N0[TiDB-A watch etcd: schemaLease 内 Reload OK] --> N1{etcd session 是否丢失}
 N1 -->|正常| N2[SchemaValidator 有效, DML 正常执行]
 N1 -->|session 超时| F1[etcd 删除 all_schema_versions/tidb-A]
 F1 --> F2[owner 不再等待 A 即可推进版本]
 F2 --> F3[A 主动 SchemaValidator.Stop, 禁 DML]
 F3 --> F4[mustRestartSyncer + mustReload + Restart 重新加入]
 F4 --> F5[恢复后 DML 放行, 期间报 Information schema is out of date]
```
...
```mermaid
flowchart TD
 A[PD Leader 宕机] --> B[etcd Raft 重新选举新 PD Leader]
 B --> C[新 Leader 读 etcd 中上一窗上界]
 C --> D{当前物理时钟 vs 窗上界}
 D -->|物理时钟更大| E[从物理时钟继续, 不回退]
 D -->|窗上界更大| F[等待物理时钟追上, 短暂取号停顿]
 F --> G[数据面: 事务 start_ts 短暂阻塞 / 延迟尖刺]
 E --> H[恢复正常取号]

 O[DDL owner 故障] --> O1[etcd owner key 失效后重选 owner]
 O1 --> O2[DDL job 继续或重试, 旧 schema 事务由 SchemaValidator 保护]

 P[OBServer 主副本切换] --> Q[ODP 路由缓存仍指旧主]
 Q --> R[SQL 发到旧主 -> 报错]
 R --> S[ODP 拉取新主位置, 更新分区 location cache]
 S --> T[重试成功, 表现为短暂错误 / 重试放大]

 U[GTS provider / LS leader 故障] --> V[Paxos/LS leader 切换]
 V --> W[GTS/SCN 不回退, 事务提交等待或重试]
```
...
sequenceDiagram
 ...
 participant TiDB as TiDB(非 owner)
 participant Owner as DDL Owner
 participant Store as 内部存储(job 队列)
 participant TiKV as TiKV

 App->>TiDB: ALTER TABLE t ADD INDEX idx(c)
 TiDB->>Store: 写入 DDL job(state=None)
 Owner->>Store: 取 job, 推进 None→DeleteOnly, ver+1
 Owner->>TiDB: 广播 schema ver, WaitVersionSynced
 Note over TiDB,Owner: 所有节点同步到 DeleteOnly 后才继续
 Owner->>Store: DeleteOnly→WriteOnly, ver+1, 等同步
 Owner->>Store: WriteOnly→WriteReorganization, ver+1, 等同步
 Owner->>TiKV: 调度 reorg/ingest worker 回填存量行索引
 TiKV-->>Owner: 回填完成(ingest 模式：有序 SST 直灌)
 Owner->>Store: WriteReorganization→Public, ver+1, 等同步
 Owner-->>App: DDL 完成, 索引可见
```
...
```mermaid
flowchart TB
 C2["Client CREATE INDEX"] --> O2["OBServer 解析/resolve"]
 O2 --> RS["RootService / DDL service"]
 RS --> SV["gen_new_schema_version"]
 SV --> DT["ObDDLScheduler 调度 index build task"]
 DT --> PX["协调快照版本 + PX/inner SQL 构建索引数据"]
 PX --> TB["Tablet / LS / checksum 校验"]
 TB --> PB["index schema 发布 public"]
 D2["并发 DML"] --> MV["multi-version schema / schema guard"]
 MV --> PX
```
...
```mermaid
flowchart TD
 A[DDL job 推进中] --> B{Owner 是否存活?}
 B -- 存活 --> C{当前 state?}
 B -- 故障 --> D[新 Owner 经 etcd 选举产生]
 D --> E[从 job 队列读未完成 job<br/>幂等续推或回滚]
 E --> C

 C -- add index 未到 Public<br/>(DeleteOnly/WriteOnly/WriteReorg) --> F[可 cancel<br/>触发回滚]
 C -- 已到 Public/done --> L[不可 cancel<br/>任务已完成]
 C -- WriteReorganization --> G{回填是否失败?}

 G -- 成功 --> H[推进至 Public, 完成]
 G -- 失败/超时 --> I[convertAddIdxJob2RollbackJob<br/>进入回滚]
 F --> I
 I --> J[delete-range 清理已写索引 KV]
 J --> K[job 标记 rollback done<br/>schema 退回旧版本]
```
...
```mermaid
flowchart TD
 S[DDL task 执行中] --> R{RootService leader 切换?}
 R -- 是 --> R1[新 RS leader 重建 ObDDLScheduler<br/>扫描未完成 ddl_task 续推/重试 ob_ddl_retry_task]
 R1 --> R2[ddl_epoch 提升<br/>旧 master 的 DDL trans 因 epoch 比对失败被拒]
 R -- 否 --> W{数据补全 wait_data_complement 失败?}
 W -- 是 --> W1[任务标记失败<br/>清理隐藏表/半成品索引 CLEANUP_GARBAGE_TASK]
 W -- 否 --> W2[checksum/report 未齐<br/>等待或返回 EAGAIN 重试]
 W2 --> E[MAKE_DDL_TAKE_EFFECT_TASK 一次性生效<br/>drop column 走 REMAP_INDEXES_AND_TAKE_EFFECT_TASK]
```
...
```mermaid
flowchart TB
 subgraph G1["record key（行 key）"]
 direction TB
 A1["前缀:t + table id"] --> A2["记录分隔:_r"] --> A3["内容:handle"]
 end
 subgraph G2["唯一索引 key（local）"]
 direction TB
 B1["前缀:t + table id + _i"] --> B2["indexID + 编码列值（memcomparable）"]
 end
 subgraph G3["非唯一索引 key（local）"]
 direction TB
 C1["t + table id + _i + indexID + 编码列值"] --> C2["内容:handle（入 key 尾保唯一）"]
 end
 subgraph G4["全局唯一索引 key"]
 direction TB
 D1["前缀:逻辑 table id + _i + indexID + 编码列值"] --> D2["PartitionIDFlag:126"] --> D3["partition id:8B"]
 end
 subgraph G5["全局非唯一索引 key"]
 direction TB
 E1["逻辑 table id + _i + indexID + 编码列值"] --> E2["PartitionIDFlag:126 + partition id:8B"] --> E3["inner handle"]
 end
 G1 ~~~ G2
 G2 ~~~ G3
 G3 ~~~ G4
...
```mermaid
flowchart TD
 A[SQL: WHERE user_id=42<br/>表按 order_id 分区] --> B{优化器选索引}
 B -->|全局索引 g_user_id| C[定位全局索引分片]
 subgraph TiDB
 direction TB
 C --> D1[扫索引 KV<br/>t&#123;tid&#125;_i&#123;idx&#125;user_id]
 D1 --> E1[解出 handle + partition id<br/>PartitionHandle]
 E1 --> F1[回表 t&#123;tid&#125;_r&#123;handle&#125;<br/>定位到具体分区 Region]
 F1 --> G1[读完整行返回]
 end
 subgraph OceanBase
 direction TB
 C --> D2[ODP 按索引表 location 路由<br/>is_index_global_=true]
 D2 --> E2[DISTRIBUTED TABLE SCAN<br/>命中索引表 Tablet/LS leader]
 E2 --> F2[ObDASLookupIter 回表<br/>带主键回主表分区]
 F2 --> G2[读完整行返回<br/>可能跨机 RPC]
 end
 G1 --> H[结果集]
 G2 --> H
...
flowchart TD
 ...
 T --> T1[全局索引 Region leader 变更]
 T1 --> T2[收到 RegionError NotLeader<br/>RegionCache 失效→向 PD 问 leader→重发回表]
 T --> T3[写入 row key 与 index key 跨 Region<br/>commit 阶段协调中断]
 T3 --> T4[读事务遇残留 primary lock<br/>CheckTxnStatus→ResolveLock 惰性推进/回滚<br/>详见第 10 章]
 T --> T5[DROP/TRUNCATE/REORGANIZE PARTITION]
 T5 --> T6[加 reorg 态扫分区逐条删全局索引项<br/>DDL 等更新完成才返回 耗时随索引数上升]

 O --> O1[全局索引表 leader 变更]
 O1 --> O2[OBServer 反馈路由失效<br/>ODP 刷新 tablet→LS 路由→重发回表]
 O --> O3[主表 Tablet 与全局索引 Tablet 跨 LS<br/>参与者 leader 切换]
 O3 --> O4[新 leader 重放 2PC 日志<br/>状态机主动驱动恢复 GTS/SCN 快照一致<br/>详见第 10 章]
 O --> O5[index build/backfill 失败或 checksum 未齐]
 O5 --> O6[RootService DDL task 重试/等待/恢复]
```
...
```mermaid
flowchart TB
 subgraph TiDB["TiDB 组件化行列分离"]
 direction TB
 APP1["客户端 SQL"]
 OPT1["优化器 CBO<br/>isolation-read engines / hint"]
 APP1 --> OPT1
 OPT1 -->|"点查/事务: 行"| TIKV[("TiKV 行存<br/>Raft Leader/Follower")]
 OPT1 -->|"大扫描/聚合/Join: 列 MPP"| TIFLASH[("TiFlash 列存<br/>Raft Learner / DeltaTree")]
 TIKV -.->|"Raft 日志异步到 Learner"| TIFLASH
 TIFLASH -.->|"读时 ReadIndex 等日志追平 + MVCC"| TIKV
 TIFLASH -->|"ExchangeSender/Receiver<br/>MinTSO 调度 + pipeline"| TIFLASH
 end

 subgraph OB["OceanBase 一体化增强"]
 direction TB
 APP2["客户端 SQL"]
 ODP["OBProxy / ODP 路由"]
 OPT2["OBServer 优化器 CBO<br/>USE_COLUMN_TABLE 提示"]
 APP2 --> ODP --> OPT2
 OPT2 -->|"TP 写/点读: 行"| MEM["MemTable 行<br/>增量"]
 OPT2 -->|"AP 扫描: 列 PX/DFO"| BASE["基线 SSTable 列<br/>major compaction 产物"]
 MEM -->|"major compaction 落列"| BASE
 BASE -.->|"查询时 on-the-fly merge"| MEM
 OPT2 -.->|"弱一致读 / COLUMN_STORE_ONLY"| CSREP[("列存副本<br/>独立 zone, 只读")]
 end
...
```mermaid
flowchart TB
 subgraph T["TiDB 故障路径"]
 direction TB
 A1["TiFlash 节点宕 / 同步落后"]
 A1 --> A2{"Learner 是否进多数派?"}
 A2 -->|"否(Learner 永不进)"| A3["TiKV quorum 提交继续<br/>OLTP 写可用性保持<br/>TiFlash replica lag 增大"]
 A1 --> A4["AP 读校验 replica 可用 / Raft index / MVCC"]
 A4 --> A5{"日志是否追平且可读?"}
 A5 -->|"否"| A6["MPP 阻塞 / warning / 报错<br/>或 engine list 允许时 fallback 到 TiKV"]
 A5 -->|"是"| A7["按 ts 过滤返回 SI 一致结果"]
 B1["MPP 大查询并发过多"] --> B2["MinTSO 仅保最小 TSO 查询调度"]
 B2 --> B3["其余查询 minTSO_wait 排队<br/>避免互相饿死"]
 end

 subgraph O["OceanBase 故障 / 异常路径"]
 direction TB
 G1["列存副本所在独立 zone 宕"]
 G1 --> G2["只读 AP 副本不可用<br/>TP 主副本 / Paxos 多数派不受影响"]
 G2 --> G3["AP 查询回落行存基线加增量<br/>(代价更高) 或按策略 reroute / 报错"]
 H1["major compaction 转列耗 CPU/IO"] --> H2{"是否做资源隔离?"}
 H2 -->|"否"| H3["与 TP 争资源<br/>OLTP 尾延迟上升(已知踩坑)"]
 H2 -->|"是"| H4["cgroup CPU/IOPS 隔离 + 资源计划压制"]
 I1["DOP 过高 / DFO 数据倾斜"] --> I2["线程调度开销、临时文件 spill、TP 资源争抢"]
 I2 --> I3["降 DOP、隔离 Tenant/ODP、rebalance leader/Unit"]
 J1["增量行积压未及时 merge"] --> J2["on-the-fly merge 变重<br/>AP 延迟随增量量上升"]
 end
...
```mermaid
flowchart TB
 subgraph G1["OceanBase 租户资源容器层级"]
 direction TB
 UC["Unit Config 资源规格模板 max_cpu/memory_size/log_disk_size/max_iops"] -->|规格| UNIT["Unit 最小资源分配单位 聚合 CPU/内存/Log Disk/IOPS"]
 UNIT -->|"1 Unit 落 1 OBServer"| POOL["Resource Pool 绑定一组 Zone"]
 POOL -->|"由若干 Unit 组成"| TEN["Tenant 独立 database service primary_zone/locality"]
 POOL -.->|"1 Pool 只授予 1 租户 / 1 租户可含多 Pool"| TEN
 TEN -->|"副本数 = Zone 数"| ZONE["Zone / Locality 决定副本分布与 Leader 落点"]
 ZONE -->|"1:N"| REP["副本 / Tablet / Log Stream-LS 归属"]
 end
```
...
```mermaid
flowchart TB
 subgraph TI["TiDB Resource Group 正常路径"]
 direction TB
 C2["客户端 / App 用户绑定 group"] --> TDB["TiDB Server 共享无状态层 解析/优化/执行计划"]
 TDB --> TB["RU 令牌桶 FillRate=RU_PER_SEC + PRIORITY"]
 TB -->|令牌足| KV["TableReader/Coprocessor 请求携带 group tag → TiKV/TiFlash"]
 KV --> SCHED["TiKV 优先级调度: PRIORITY / RU 比例"]
 SCHED --> RDB["共享 RocksDB"]
 TDB -.Placement Rules.-> PLC["约束部分库表副本位置/拓扑"]
 PLC --> KV
 TDB -.RU 计量.-> RUT["mysql.request_unit_by_group"]
 end

 subgraph OB["OceanBase 用户租户正常路径"]
 direction TB
 C1["客户端 / App"] --> ODP["OBProxy/ODP 路由到 tenant"]
 ODP --> OMT["omt: ObMultiTenant 时间片调度"]
 OMT --> TEN["ObTenant 独立线程池/内存上下文 (user tenant session)"]
 TEN --> CG["CPU: cgroup cpu.shares/cfs_quota + worker=2+unit_min_cpu×cpu_quota_concurrency"]
 TEN --> MEM["内存: 租户物理隔离硬上限"]
 TEN --> IO["IO: ObTenantIOClock 三参数(MIN/MAX/WEIGHT IOPS)"]
 IO --> LS["Tablet / 独立 Log Stream-LS / 日志盘"]
 LS --> LOC["Locality / Primary Zone 决定副本与 leader 偏好"]
 TEN -.元数据/恢复信息.-> META["配对 meta 租户(tenant_id=user-1)"]
 end
...
```mermaid
flowchart TB
 T1["热点业务 / 噪声邻居出现"] --> T2{"TiDB 路径"}
 T2 --> T3["邻 group RU 桶耗尽 (非 BURSTABLE→等待回填; BURSTABLE→可超用)"]
 T3 --> T4["请求排队 / 降优先级 / Runaway: DRYRUN·COOLDOWN·KILL·SWITCH_GROUP(v8.4)"]
 T4 --> T5["无独立内存: 写放大/热点 Region/磁盘队列仍可波及共享 RocksDB 与本 group"]
 T5 --> T6["事后纠偏: 扩容 / 重配 RU / 调 Placement / 拆集群"]

 T1 --> O2{"OceanBase 路径"}
 O2 --> O3["邻租户 Unit IOPS/log disk/memory 达边界"]
 O3 --> O4["内存硬墙: 邻租户 OOM 自报错, 不挤占本租户; 后台任务排队"]
 O4 --> O5["RootService / Unit balance / merge / transfer 介入"]
 O5 --> O6["本租户 P99 基本稳; 必要时调 Unit/Resource Pool/Locality 或迁移租户"]

 C1["cgroup 相关异常 (权限缺失/容器无 cgroup)"] --> C2["enable_cgroup × is_valid() 四象限 → check_cgroup_status() 动态降级"]
 C2 --> C3["隔离退化为 进程内时间片 + 三参数 IO 调度, observer 继续服务 (强度下降, 不中断)"]
 C3 --> C4["注: cgroup 自 V4.0 起即可关闭; Cloud 形态的'不依赖 cgroup'不可泛化到 CE 所有场景"]

 M1["major freeze / merge 抖动 (OceanBase)"] --> M2["sys 租户 RootService 选全局版本 → 各租户 major merge"]
 M2 --> M3["merge IO/CPU 压力 → MIN_IOPS(reservation) 保底前台 + enable_global_background_resource_isolation(4.4) 切后台 vCPU"]
 M3 --> M4["仍可能瞬时抖动 → 经增量/渐进/轮转合并 + IO 隔离缓解"]
```
...
```mermaid
flowchart TB
 subgraph R1["地域 1 / Seattle 同城（多数派边界:同城 2 AZ=4 副本）"]
 direction TB
 subgraph Z1["AZ1"]
 direction TB
 A1["Region Leader（Leader 仅限同城两 AZ）"]
 A2["Voter 副本"]
 end
 subgraph Z2["AZ2（AZ1↔AZ2 同城 <3ms）"]
 direction TB
 B1["Voter 副本"]
 B2["Voter 副本"]
 end
 ...
 subgraph R2["地域 2 / San Francisco 异地（异地↔同城 约 20ms 专线）"]
 direction TB
 C1["AZ3:1 副本（城市级容灾保底投票者，不进多数派关键路径）"]
 end
 R1 ~~~ R2
```
...
```mermaid
flowchart TD
 Client[客户端] -->|SQL 写| TiDB[TiDB Server 主中心]
 TiDB -->|取 TSO| PD[(PD / TSO / Placement)]
 TiDB -->|2PC prewrite/commit| Leader[Region Leader<br/>主中心 AZ1]
 Leader -->|Raft 日志复制| F1[Follower 主中心 AZ1]
 Leader -->|Raft 日志复制 跨AZ <1.5ms| V2[Voter 灾备中心 AZ2]
 Leader -.->|异步| L2[Learner 灾备中心 AZ2]
 F1 -->|ack| Leader
 V2 -->|ack 跨AZ| Leader
 Leader -->|两中心均确认 多数派提交| Commit{提交}
 Commit -->|RPO=0| TiDB
 PD -.->|监控副本分布 维持 SYNC 状态| Leader
 TiDB -->|返回| Client
```
...
```mermaid
flowchart TB
 C2["客户端 / OBProxy"] --> OBS["OBServer"]
 OBS --> RS["RootService / Tenant 元数据"]
 OBS --> LS["LS Leader"]
 LS --> P1["Paxos full 副本"]
 LS --> P2["full / 仲裁副本"]
 LS --> ARC["日志归档 / 备份介质"]
 ARC --> STBY["物理 standby 租户"]
```
...
```mermaid
flowchart TD
 Start[sync 状态<br/>两中心强同步] -->|灾备中心 AZ2 网络隔离| Detect{PD 检测<br/>drHasVoter=false?}
 Detect -->|未超 wait-store-timeout 60s| Wait[async_wait<br/>内部过渡态 等待恢复]
 Wait -->|网络恢复| Start
 Wait -->|超 60s 仍失联| Async[async 状态<br/>退回 majority<br/>仅主中心凑多数派]
 Async -->|此时仍有多数派| Serve[主中心继续服务<br/>但 RPO 不再=0]
 Async -.->|若主中心再挂过半 voter| Fatal[多数派丢失<br/>集群不可用<br/>降级也救不了]
 Async -->|灾备网络恢复| Recover[sync_recover<br/>逐步追平日志<br/>ACIDConsistent=false]
 Recover -->|追平完成| Start

 subgraph OB[OceanBase 2F1A 对照]
 direction TB
 OBStart[2F1A 正常<br/>仲裁成员不投日志] -->|1 个 full 副本故障| OBTimeout{仲裁超时内<br/>无日志确认?}
 OBTimeout -->|是| OBDegrade[仲裁服务降级故障成员<br/>剩 1F+1A 仍服务 RPO=0]
 OBDegrade -->|另一 full 也挂| OBFatal[LS 无主<br/>仲裁成员无法独自恢复]
 end
```
...
```mermaid
flowchart TD
 F["故障:节点 / AZ / 网络分区"] --> M{"多数派仍可达?"}
 M -- "是" --> E["选主或 Leader 转移"]
 E --> R["客户端路由刷新 / OBProxy location 刷新"]
 R --> S["服务恢复，伴短暂尾延迟"]
 M -- "否" --> P{"是否存在同步灾备完整数据?"}
 P -- "有，但原多数派已丢失" --> H["人工/工具辅助恢复，数据校验，租户/集群角色切换"]
 P -- "无或未知" --> X["停止自动写入;选 unsafe recovery 或从备份/PITR 恢复"]
 X --> B["校验归档/日志备份 checkpoint"]
 B --> CC["恢复到污染前的 SCN/timestamp/checkpoint"]
 H --> V["演练、对账、可控回切"]
 CC --> V
```
...
```mermaid
flowchart TB
 subgraph TiDB["TiDB 观测数据流(正常路径)"]
 direction TB
 App1[应用 SQL] --> TS[TiDB Server]
 TS -->|执行信息 StmtExecInfo| SS[Statement Summary<br/>LRU / 按 digest 聚合]
 TS -->|耗时 > 300ms| SL[Slow Query Log<br/>SLOW_QUERY 表]
 TS -->|CPU 采样| TOP[Top SQL<br/>CPU 按 SQL 聚合]
 TS -->|metrics| PROM[Prometheus]
 TKV1[TiKV] -->|metrics + RocksDB| PROM
 PD1[PD] -->|metrics + 热点| PROM
 PROM --> GRAF[Grafana<br/>Performance Overview]
 GRAF -->|database time 下钻| OPS1[运维者]
 SS --> DASH[TiDB Dashboard]
 TOP --> DASH
 DASH --> OPS1
 end

 subgraph OB["OceanBase 观测数据流(正常路径)"]
 direction TB
 App2[应用 SQL] --> ODP[ODP]
 ODP --> OBS[OBServer 内核]
 OBS -->|record_request| AUDIT[GV$OB_SQL_AUDIT<br/>内存环形缓冲]
 OBS -->|SHOW TRACE / trace id| TRACE[SQL Trace 阶段耗时]
 OBS -->|1s 采样| ASH[GV$ACTIVE_SESSION_HISTORY]
 OBS -->|compaction 状态| COMP[GV$OB_COMPACTION_PROGRESS]
 OBS -->|租户内存| MEM[GV$OB_TENANT_MEMORY]
 OBS -->|metrics| OBAGENT[OBAgent] --> PROM2[Prometheus / OCP]
 AUDIT -->|SQL 直查| OPS2[运维者]
 TRACE --> OPS2
 ASH --> OPS2
 PROM2 --> OPS2
 end
...
```mermaid
flowchart TD
 p99["P99 抖动告警"] --> split{"分层定位"}
 split --> client["客户端 / 连接池 / ODP 或 LB"]
 split --> sql["SQL 层: 计划变化 / 慢 SQL / 锁等待"]
 split --> store["存储层: Region 热点 / LS 热点 / RocksDB stall / merge"]
 split --> ctrl["控制面: PD 调度 / RootService 任务 / TSO 或 GTS 依赖"]
 client --> evidence
 sql --> evidence["关联键: SQL digest / trace id / slow log / SQL Audit"]
 store --> evidence
 ctrl --> evidence
 evidence --> logs["组件日志 + Diag / obdiag 诊断包"]
 logs --> action["处置: 限流 / 打散热点 / 错峰 merge / 调资源 / 修正计划 / 回滚"]
 action --> verify["回看同一时间窗: P99 / 错误率 / 重试 / 后台任务是否回落"]
```
...
flowchart TB
 subgraph L1["第1层　协议层"]
 direction TB
 L1A["TiDB：MySQL 协议，MySQL 客户端/驱动可直连"] ~~~ L1B["OceanBase：MySQL 协议；Oracle 模式另有 Oracle 协议面"]
 end
 subgraph L2["第2层　方言层（grammar）"]
 direction TB
 L2A["TiDB：仅 MySQL 单一方言，无 Oracle 模式概念"] ~~~ L2B["OceanBase：租户级 MySQL/Oracle 二选一；CE 仅 MySQL 模式语法前端"]
 end
 subgraph L3["第3层　执行层（planner/executor）"]
 direction TB
 L3A["TiDB：存储过程解析成 ProcedureInfo 但 executor 无接入，不可执行"] ~~~ L3B["OceanBase：PL 引擎真执行（编译/代码生成/包/异常）"]
 end
 subgraph L4["第4层　语义层"]
 direction TB
 L4A["TiDB：默认 sql_mode 七项；REPEATABLE-READ 名义下为 Snapshot Isolation"] ~~~ L4B["OceanBase：CompatMode 创建时定不可改；非法日期需 _enable_mysql_compatible_dates"]
 end
 subgraph L5["第5层　发布/授权层"]
 direction TB
 L5A["TiDB：全开源（外键 v8.5.0 GA、存储过程/触发器均开源）"] ~~~ L5B["OceanBase：CE 仅 MySQL 模式；Oracle 模式属企业版/Cloud"]
 end
 L1 ~~~ L2
 L2 ~~~ L3
 L3 ~~~ L4
...
```mermaid
flowchart TB
 subgraph TI["TiDB 单方言正常路径"]
 direction TB
 C1[MySQL 客户端/驱动] --> PROTO1[MySQL 协议握手/认证]
 PROTO1 --> P1["MySQL 模式 parser 唯一方言"]
 P1 --> SEM1["语义: sql_mode 默认 7 项 / 隔离级名义 REPEATABLE-READ"]
 SEM1 --> EX1{语句类型与执行器接入?}
 EX1 -->|DML/JSON| RUN1["planner→executor 读写 TiKV/TiFlash"]
 EX1 -->|外键 DML| FK1["父子表检查/级联动作/DDL 状态机"]
 EX1 -->|DDL| DDL1["DDL owner / schema version 推送"]
 EX1 -->|存储过程| STUB1["仅解析成 ProcedureInfo, 无执行→不可运行"]
 EX1 -->|触发器| NONE1["无 CREATE 语法→直接报错"]
 end

 subgraph OB["OceanBase 双模式正常路径"]
 direction TB
 C2[客户端连某租户] --> MODE{租户 CompatMode?}
 MODE -->|MYSQL=0| PM["MySQL 模式 session/parser(CE 仅此)"]
 MODE -->|ORACLE=1| PO["Oracle 模式 session/parser(企业版/Cloud)"]
 PM --> RES["共享 resolver: is_oracle_mode() 分支(约 799 文件)"]
 PO --> RES
 RES --> OBJ["对象层: SEQUENCE/SYNONYM/PACKAGE/TRIGGER resolver"]
 OBJ --> PL["PL 引擎: 编译/codegen/包/异常 真执行"]
 PL --> STORE["对象字典 / Tablet / Log Stream-LS / 事务与锁"]
 end
...
```mermaid
flowchart TD
 A[迁移后业务 SQL] --> B{命中兼容性边界?}
 B -->|"不支持语法 (TiDB 存储过程/触发器)"| C["解析/resolver 报错或解析成功却无执行器→上线才发现失效"]
 B -->|"语法可解析但语义不同"| D["计划/锁/事务行为偏差: sql_mode/非法日期/RR 名义下的 SI/NULL 排序/错误码"]
 B -->|"DDL 或对象差异"| E["schema migration / online DDL 中断 / 模式不可改"]
 B -->|"触发器/PL/外键副作用"| F["额外写入 / 级联 / 大事务 / 回滚"]
 D --> G["并发测试暴露: 写偏斜/锁等待/更新失败/错误码不一致"]
 E --> H["工具链处理: DM/OMS/TiCDC/BR/Lightning 限制"]
 F --> I["审计: 日志/事务状态/约束错误/回放一致性"]
 G --> J[修订 SQL / 应用逻辑 / 迁移规则]
 H --> J
 I --> J
```
...
```mermaid
flowchart TB
 subgraph TiDB["TiDB 正常安全路径(classic / Cloud)"]
 direction TB
 C1["客户端"] -->|"1. TLS / MySQL 协议<br/>(可选 SM3 / JWT 认证、REQUIRE SSL)"| T1["TiDB Server"]
 T1 -->|"2. 认证 user@host / plugin"| TA["Auth"]
 TA -->|"3. 权限校验<br/>静态位 + 动态权限 + role + SEM"| T2["Privilege Check"]
 T2 -->|"4. 审计回调<br/>AuditManifest(企业 / Cloud)"| T3["Audit Plugin / Cloud Audit"]
 T2 -->|"5. 事务 / 2PC"| T4["TiKV Region"]
 T4 -->|"6. 信封加密落盘<br/>data key 加密文件"| T5[("RocksDB<br/>AES / SM4-CTR")]
 K1["KMS / File<br/>master key"] -.->|包裹 data key| T4
 T2 -->|"备份链路"| BR1["BR backup"]
 BR1 -->|client-side 加密| K1
 BR1 --> S1["对象存储"]
 end

 subgraph OB["OceanBase 正常安全路径"]
 direction TB
 C2["客户端"] -->|"1. SSL / MySQL 协议<br/>(OBProxy 透传、OB-RPC 节点间)"| O1["ODP / OBServer"]
 O1 -->|"2. 进入 Tenant 上下文"| TEN["Tenant context"]
 TEN -->|"3. ObPrivilegeCheck<br/>租户 + 双模权限"| O3["Privilege Check"]
 O3 -->|"4. Security Audit / SQL Audit<br/>check_allow_audit"| O4["GV$OB_SQL_AUDIT / Audit Trail"]
 O3 -->|"5. SQL 执行 → 复制式 WAL / PALF append + 事务状态推进"| O5["Tablet / Log Stream-LS"]
 O5 -->|"6. TDE 表空间加密<br/>tablespace key"| O6[("SSTable / clog<br/>AES / SM4")]
 K2["Tenant master key<br/>keystore / KMS"] -.->|加密 tablespace key| O5
 end
...
```mermaid
flowchart TD
 S["安全异常"] --> TLS["证书过期 / CA 不匹配 / AutoTLS=false 未配证书"]
 TLS --> R1["连接失败，或明文传输 / 中间人风险"]
 S --> PRIV["权限撤销滞后 / role 未启用 / SEM 误配"]
 PRIV --> R2["命令被错误放行，或 SUPER 误以为万能被拒"]
 S --> AUD["审计关闭 / filter 过窄 / 存储不可写 / 内存视图淘汰"]
 AUD --> R3["取证缺口(开源无插件、GV$OB_SQL_AUDIT 被冲)或敏感日志泄露"]
 S --> KMS["KMS 不可达 / 主密钥丢失 / key policy 收紧"]
 KMS --> R4["TDE / BR / 恢复无法解密；主密钥丢失即永久不可恢复"]
 S --> TEN["sys 租户 / Cloud 管理员凭据过宽或泄露"]
 TEN --> R5["跨租户运维越界"]
 S --> BAK["备份密文与密钥同域存放"]
 BAK --> R6["离线备份可被旁路解密"]
 KMS --> VER["LS Leader 切换后取 master_key_version"]
 VER --> R7["版本元数据滞后 → 解密失败 / 读阻塞(虚表可观测)"]
 TLS ~~~ PRIV
 PRIV ~~~ AUD
 AUD ~~~ KMS
 KMS ~~~ TEN
...
```mermaid
flowchart TB
 R["读 /proc/self/cgroup(V1) 或 /proc/self/mountinfo(V2)"] --> M["InContainer() 匹配 docker / kubepods / containerd"]
 M --> J{命中任一}
 J -->|是| IN["判定在容器内"]
 J -->|否| OUT["非容器环境"]
 IN --> CPU["CPU 支:读 cgroup CPU quota"]
 IN --> MEM["内存支:cgroup V2 读 memory.max"]
 CPU --> AMP["automaxprocs(maxprocs.Set) 映射"]
 AMP --> GMP["设置 GOMAXPROCS"]
 MEM --> FAC["memory.max × 0.9 防 OOM"]
 GMP --> CG["cgmon 周期 refreshInterval = 10s 刷新"]
 FAC --> CG
 CG --> EXP["暴露 Prometheus 指标 tidb_server_maxprocs"]
```
...
```mermaid
flowchart TB
 U["DBA / 平台 API"] --> K8S["Kubernetes API Server"]
 K8S --> TC["TidbCluster / Backup / Restore CR"]
 K8S --> OBC["OBCluster / OBTenant / Backup CR"]

 subgraph TiDBPlane["TiDB 控制面 + 数据面"]
 direction TB
 TC --> TCM["tidb-controller-manager<br/>(reconcile 循环)"]
 TCM -->|生成/更新| TSS["StatefulSet / Deployment / Service / Job"]
 TSS --> TPOD["TiDB Server Pods<br/>(无状态 SQL)"]
 TSS --> KVPOD["TiKV Pods + PVC<br/>(Region / Raft Group)"]
 TSS --> PDPOD["PD Pods + PVC<br/>(TSO / 调度 / 元数据)"]
 Client["客户端 / 应用"] -->|SQL| TPOD
 TPOD -->|取 TSO / RegionCache 路由| PDPOD
 TPOD -->|读写 KV| KVPOD
 KVPOD <-->|Raft 复制| KVPOD
 TCM -->|升级前调 PD<br/>evict Region Leader| PDPOD
 end

 subgraph OBPlane["OceanBase 控制面 + 数据面"]
 direction TB
 OBC --> OBOP["ob-operator resource managers"]
 OBOP -->|直管| OBK8S["OBServer Pods / PVC / Service"]
 OBK8S --> OBS["OBServer 进程"]
 OBS --> RST["RootService / sys tenant"]
 RST --> TEN["Tenant / Unit / Resource Pool"]
 TEN --> LS["Log Stream / LS / Tablet"]
 end
...
```mermaid
flowchart TD
 N["Node 故障或维护<br/>(承载 tikv / OBServer + 其 local PV)"] --> P["Pod 失联 / 重建<br/>(store Disconnected / OBServer 异常)"]
 P --> V{PV 类型}
 V -->|cloud disk 可重新 attach| A["新节点挂载 PVC<br/>→ 数据库进程启动"]
 V -->|local PV 绑原节点| L["nodeAffinity 锁定原节点<br/>Volume Node Affinity Conflict<br/>替补 Pod 卡 Pending"]
 A --> S{数据库内部状态}
 L --> H["靠共识在健康节点新建副本<br/>原节点不回则旧 PV 数据永久不可用"]
 H --> S
 S -->|TiDB| T1["PD 检查 member / store / Region<br/>30m 转 Down + 5m failover → 补副本 / 切 Leader"]
 S -->|OceanBase| O1["RootService / sys tenant 检查<br/>OBServer / Unit / Tenant / LS"]
 T1 --> OK["服务恢复或降级运行"]
 O1 --> OK
 T1 --> RISK["旧 PVC 误用 / quorum 不足 / 恢复需人工"]
 O1 --> RISK
```
...
```mermaid
flowchart TB
 subgraph TiDBClassic["TiDB classic OLTP（shared-nothing）"]
 direction TB
 C1["Client"] --> T1["TiDB Server"]
 T1 --> P1["PD: Region 路由 / TSO / 调度"]
 T1 --> K1["TiKV Region leader"]
 K1 --> R1["Raft peers on TiKV Stores"]
 R1 --> L1["本地 RocksDB / Raft Engine"]
 end

 subgraph TiFlashDisagg["TiFlash disaggregated AP（真 shared-storage）"]
 direction TB
 T1 --> M1["MPP / cop task"]
 M1 --> CN["TiFlash Compute Node（无状态）"]
 CN --> LC["Compute 本地 remote cache"]
 CN --> S3A[("S3 / S3 兼容对象存储")]
 K1 --> WN["TiFlash Write Node（Learner）"]
 WN --> S3A
 end

 TiDBClassic ~~~ TiFlashDisagg
```
...
```mermaid
flowchart TD
 Client["客户端 DML（经 OBProxy/ODP）"] --> RW["计算节点 RW: log stream leader"]
 RW --> MEM["写入 MemTable 内存"]
 RW --> CLOG["CLog 先写本地缓存/云盘 低延迟"]
 CLOG --> PALF["服务化 PALF 日志服务<br/>LogServer 三副本 Paxos 共识"]
 PALF -->|多数派持久化| COMMIT["事务提交返回"]

 MEM -->|达阈值冻结| MINI["mini/micro compaction<br/>生成 mini/micro SSTable"]
 MINI --> LCACHE["先落计算节点本地盘缓存"]
 LCACHE -->|后台异步| SSW{"SSWriter 单写者租约<br/>该 log stream 唯一写者"}
 SSW -->|上传 SHARED_* 宏块| OBJ[("对象存储 S3/OSS/Blob<br/>唯一真相源")]

 OBJ -->|minor/major compaction<br/>卸载到共享存储层| MERGE["共享存储层 compactor<br/>major 7 阶段"]
 MERGE -->|合并后| OBJ
 MERGE -->|预热| DCACHE["分布式缓存 BlockServer<br/>AZ 级共享 macro-block"]

 RW -.读命中.-> MEM
 RW -.读命中.-> LCACHE
 RW -.warm.-> DCACHE
 RW -.cold miss 100ms+.-> OBJ
```
...
```mermaid
flowchart TD
 A["异常: compute / cache / object storage / control plane 抖动"] --> B{路径类型}

 B --> C["TiDB classic: TiKV Store 或 Region leader 故障"]
 C --> C1["Raft leader election / peer catch-up"]
 C1 --> C2["PD 依据 heartbeat 和 placement 继续调度"]
 C2 --> C3["风险：Region 迁移、snapshot、compaction 与前台写争资源"]

 B --> D["TiFlash disaggregated: Compute Node 故障"]
 D --> D1["TiDB 失效/刷新 tiflash_compute 拓扑"]
 D1 --> D2["MPP task retry 到其他 Compute Node"]
 D2 --> D3["风险：cache miss 暴露 S3 / Write Node 状态"]

 B --> E["Bacchus / OceanBase SS: RW/RO 或 cache service 故障"]
 E --> E1["经 PALF shared log 与 metadata 恢复进度"]
 E1 --> E2["local cache miss → distributed cache → object storage"]
 E2 --> E3["风险：对象存储尾延迟、cache warm-up、compaction backlog"]
```
...
```mermaid
flowchart TD
 subgraph 故障1["Leader 故障 / log stream 切主"]
 direction TB
 A1["RW leader 宕机"] --> A2["follower 经 PALF 共识选新 leader"]
 A2 --> A3["新 leader 重设某副本为 SSWriter<br/>旧 SSWriter 租约自然过期"]
 A3 --> A4["Leader/Follower 缓存预热<br/>按访问序列追平 local cache"]
 end

 subgraph 故障2["计算节点冷启动 / scale-out"]
 direction TB
 B1["新计算节点加入：缓存全空"] --> B2["读全部穿透对象存储 100ms+"]
 B2 --> B3["Replication Migration 预热<br/>从 BlockServer 取增量 + 共享存储取基线"]
 B3 --> B4["命中率回升，延迟回落"]
 end

 subgraph 故障3["对象存储不可用 / 限流"]
 direction TB
 D1["对象存储 throttle / p99 尖刺"] --> D2["穿透读尾延迟放大"]
 D2 --> D3["依赖多级缓存吸收，但 cold miss 无法避免"]
 end

 subgraph 故障4["SSWriter 租约异常"]
 direction TB
 F1["SSWriter 网络分区"] --> F2["租约过期，停止写对象存储"]
 F2 --> F3["其他副本检测过期租约后接管<br/>防止双写者并发写同一对象"]
 end
...
flowchart TB
 subgraph CTRL["控制面"]
 direction TB
 master["DM-master:任务调度 / DDL 协调"]
 dmctl["dmctl:运维入口"]
 dmctl --> master
 end
 subgraph DATA["数据面"]
 direction TB
 up["上游多分片表（经 route rules 合并到下游同一逻辑表）"]
 worker["DM-worker"]
 relay["relay log（本地，多同上游任务复用）"]
 dump["dump phase:全量导出"]
 load["load phase:导入下游"]
 sync["sync phase:解析上游 binlog 增量复制（safe mode 作用于此）"]
 down["下游 TiDB:同一逻辑表"]
 up --> worker
 worker --> relay
 relay --> dump
...
```mermaid
flowchart TB
 subgraph BITS["AUTO_RANDOM 64 位行键位域布局(高位→低位)"]
 direction TB
 F1["bit 63:符号位"] --> F2["保留/range 区:宽 64−R 位（range bits R∈[32,64]，默认/max 64、min 32）"] --> F3["分片位 S:默认 5（2⁵=32 路打散）、max 15"] --> F4["自增位:最小 27 位（AutoRandomIncBitsMin）"]
 end
```
...
```mermaid
flowchart TD
 A[评估阶段<br/>schema/SQL/分片键/ID/历史库<br/>主键索引分区目标模型设计] --> B[结构迁移<br/>DM schema route rules / OMS Schema Migration]
 B --> C[全量迁移<br/>Dumpling+Lightning / OMS Full Migration<br/>记录源位点与目标 schema 版本]
 C --> D[增量同步<br/>DM sync binlog / OMS Incremental Sync]
 D --> E[数据校验<br/>sync-diff-inspector 映射校验 + 业务对账<br/>OMS Full Verification]
 E --> F{校验一致?}
 F -- 否 --> D
 F -- 是 --> G[双写灰度<br/>应用双写源库+新库<br/>幂等并记录冲突]
 G --> H[灰度切读<br/>按比例切读流量到新库]
 H --> I{性能/正确性回归通过?}
 I -- 否 --> R[回退读流量到源库]
 I -- 是 --> J[切写<br/>写流量切到新库]
 J --> K[反向增量<br/>新库->源库<br/>保持源库可回退]
 K --> L[观察期稳定后<br/>下线源库与反向链路]
```
...
flowchart TD
 S0[切流后运行中] --> S1{异常类型}
 S1 -->|新库写热点| H1[AUTO_RANDOM未生效/分区键选错<br/>单Region/单Tablet排队]
 S1 -->|跨片事务超时| H2[原应用层拆分事务合并后<br/>跨Region/跨LS 2PC<br/>多数派往返延迟上升]
 S1 -->|全局索引写慢| H3[非分片键唯一约束<br/>写入升级分布式事务<br/>写放大]
 S1 -->|上游分片DDL不一致| H4[DM sharding DDL lock/<br/>OMS schema 转换失败<br/>同步中断]
 S1 -->|主键唯一键冲突| H5[auto-increment 重叠/<br/>旧全局 ID 漏洞/双写竞态]
 S1 -->|增量延迟扩大| H6[binlog/同步位点落后<br/>积压]

 H1 --> R1[在线加SHARD_ROW_ID_BITS需重建/<br/>OB调整分区键<br/>限速回填与回放压测]
 H2 --> R2[用Table Group共置/<br/>调整分区键本地化事务]
 H3 --> R3[评估改局部索引/<br/>下沉唯一性到应用幂等表]
 H4 --> R4[暂停对应迁移组<br/>人工改写DDL/必要时重做该组]
 H5 --> R5{可修复?}
 H6 --> R6[延后切流<br/>扩容/限流/拆分大事务]
 R5 -->|是| FX[sync-diff生成修复SQL/<br/>OMS重新校验]
 R5 -->|否 切流后| RB[触发回滚<br/>读写切回源库<br/>依赖反向增量已建立]
 RB --> RB2[反向链路把新库增量<br/>回灌源库后停新库]
 R4 --> M[重新校验]
 R6 --> M
 FX --> M
```
...
```mermaid
flowchart TB
 subgraph Plan[测试规划:固定五前提]
 direction TB
 P1[workload: TPC-C/sysbench/CH] --> P2[硬件拓扑对齐]
 P2 --> P3[版本锁定 TiDB8.5 / OB4.2.5]
 P3 --> P4[配置对齐:两侧均用官方推荐]
 P4 --> P5[多连接并发,避免压测工具自身瓶颈]
 end
 Plan --> Load[装载阶段 prepare]
 Load --> Warm[预热:等 compaction/merge 稳定]
 Warm --> Run[正式压测 run,多档并发]

 subgraph TiDB_Path[TiDB 数据/控制路径]
 direction TB
 Run --> T1[go-tpc -> TiDB Server 无状态]
 T1 --> T2[client-go 路由 Region/Leader]
 T2 --> T3[Percolator 2PC + Region Raft apply]
 T3 --> T4[TiKV RocksDB 落盘]
 T2 -. CH-benCHmark .-> T5[TiFlash learner / MPP]
 end
 subgraph OB_Path[OceanBase 数据/控制路径]
 direction TB
 Run --> O1[obtpcc -> OBProxy/ODP 路由]
 O1 --> O2[OBServer 分区键->Tablet->LS]
 O2 --> O3[树形2PC + LS PALF 复制]
 O3 --> O4[MemTable/SSTable + merge 落盘]
 end

 subgraph Observe[同步采集:不只看 TPS]
 direction TB
 T4 --> M1[Prometheus: apply lag / write stall / GC / TSO]
 O4 --> M2[gv$: SQL_AUDIT / compaction / log_stat]
 M1 --> R[报告:TPS + P99/P999 + 抖动归因]
 M2 --> R
 end
 R --> Repro[复现性:记录硬件/版本/配置/前提]
...
```mermaid
flowchart TD
 C[压测运行中,稳态 TPS=X] --> D{注入故障}
 D --> D1[TiKV / OBServer leader kill 或 pause]
 D --> D2[网络延迟 / 分区 / 丢包]
 D --> D3[磁盘 IO 抖动 / compaction 或 merge 放大]
 D --> D4[PD/TSO 或 RootService/GTS 依赖抖动]
 D --> D5[DDL / add index / rebalance 并发]
 D --> D6[时钟漂移 TimeChaos]

 D1 --> E[TiDB Leader 重选 / OB LS Paxos 切主,路由缓存失效,重试]
 D2 --> F[跨 Region 或跨 LS 事务等待]
 D3 --> G[P95/P99/P999 长尾上升]
 D4 --> H[新事务取时间戳或元数据变更受阻]
 D5 --> I[后台任务与前台 workload 争抢 CPU/IO/日志]
 D6 --> K[TSO/GTS 时序扰动,验证 SI 是否破坏]

 E --> J[记录可用性窗口 / 错误码 / RTO / RPO / tpmC 恢复曲线]
 F --> J
 G --> J
 H --> J
 I --> J
 K --> J
 J --> V[Jepsen/Elle 校验:历史是否仍满足 SI / 单键线性一致]
```
